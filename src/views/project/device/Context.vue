...
import Import from '@/views/project/device/components/import'
import {
  getDeviceList,
  deleteMethod,
  addMethod,
...
function getList() {
  loading.value = true
  const deviceTypes = active.value === 'mqtt' ? [10, 20, 40] : [30]
  getDeviceList({ ...queryParams.value, deviceTypes }).then((res) => {
...
const onTableClick = async (row, key, readWriteFlag, params, cmdCode) => {
  ...
  const res = await editOtherInfoMethod([postValue], active.value)
  if (res.code === 200) {
    proxy.$modal.msgSuccess('操作成功')
...
const issueClick = async (item) => {
  ...
  const res = await editOtherInfoMethod(list, active.value)
  if (res.code === 200) {
